...
export default async function DetailsManga({params}: {params: {id: number}}) {
                ...
                <div className=" p-6 flex flex-col items-start gap-4 rounded-lg shadow-lg max-w-7xl w-full">
                    <div className="flex flex-col md:flex-row items-center md:items-start">
                        {manga?.images?.jpg?.image_url ? (
                            <img src={manga.images.jpg.image_url} alt={manga.title} className="w-full h-fit rounded-lg mb-4 md:mb-0 md:mr-6"/>
                        ) : (
                        ...
                        <div className="text-white">
                            ...
                            <p className="mb-3"><span className="font-semibold text-[#32cd87]">Score:</span> {manga?.score}</p>
                            <p className="mb-3"><span className="font-semibold text-[#32cd87]">Rank:</span> {manga?.rank}</p>
                            <div className="mb-3">
                                <span className="font-semibold text-[#32cd87]">Genres:</span> {manga.genres?.map((genre:any) => genre.name).join(', ')}
                            </div>
                            <p className="mt-4 text-[#a5a5a5] text-sm font-light">{manga.synopsis}</p>
                        </div>
                    </div>
