...
const HeroSection = () => {
    ...
    const buttonText = query ? `Search` : "Search ";
  ...
  const  card = [
    {
        name: 'Ella Marie', about:'Very Interesting Website, Love it', description: 'Creative Director',img:'https://res.cloudinary.com/datm8nslc/image/upload/v1746221984/images8_c4eus0.jpg'
    },
     {
        name: 'Sam Smith', about:'So much to explore ,very enjoyable', description: 'Youtube Creator', img:'https://res.cloudinary.com/datm8nslc/image/upload/v1746222025/images_5_q74apj.jpg'
    },
     {
        name: 'Ella Marie', about:'I could spent days here ,endless joy', description: 'Creative Director',img:'https://res.cloudinary.com/datm8nslc/image/upload/v1746222033/images_9_rgjjwh.jpg'
    }
      ...
      <div className="wrapper z-100 absolute top-0 left-0 w-3/6 h-screen">
       ...
       <div className="text-[32px] md:text-[48px] lg:text-[80px] z-100 text-white font-[500] w-full lg:w-3/4 lg:leading-[105px] text-balance mt-14">Your Gateway to Anime Worlds    </div>
        ...
        <button
            type="submit"
            className="px-4.5 py-3 font-medium cursor-pointer text-sm sm:text-md -ml-12 sm:-ml-0 bg-[#245F37] text-[#fff] rounded-xl  transition"
        >
            {buttonText}
...
const Card = ({card}:CardProps)=>{
 ...
 useEffect(() => {
    ...
    exit={{ opacity: 0, y: -20 }}
    transition={{ duration: 0.5, ease: "easeInOut" }}
    className="text-fade text-white text-lg font-medium">{currentCard.about}</motion.h1>
        <div className="flex items-center gap-2 rounded-full p-1.5 bg-[#0E0E0E]">
            <img src={currentCard.img} className="w-10 h-10 rounded-full object-cover" alt="" />
            <p className="text-white text-md font-medium">{currentCard.name}</p>
            <span className="text-[#a5a5a5] text-sm font-medium">{currentCard.description}</span>
        </div>
        </motion.div>
        </AnimatePresence>
        </>
