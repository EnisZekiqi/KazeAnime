...
export default function useFavorites<T extends { mal_id: string | number; type?: string }>(defaultType?: string) {
    ...
    const addFavorite = (item: T) => {
        const itemType = (item as any).type ?? defaultType ?? 'unknown';
        const itemWithType = { ...(item as any), type: itemType } as T & { type: string };
